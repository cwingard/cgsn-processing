...
IN="$DATA/proc/$PLATFORM/$DEPLOY/$FB250/$FILE"
OUT="$DATA/erddap/$PLATFORM/$DEPLOY/$FB250/${FILE%.json}.nc"
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_syslog_fb250 -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LNG -i $IN -o $OUT
fi
