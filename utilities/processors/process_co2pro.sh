...
if [ $# -ne 7 ]; then
    ...
    echo ""
    echo "     example: $0 ce02shsm D00018 44.63929 -124.30404 nsif/pco2test 7 20161012.pco2test.json"
    exit 1
fi
PLATFORM=${1,,}
DEPLOY=${2^^}
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_mopak -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LON -dp $DEPTH \
