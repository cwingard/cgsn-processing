...
if [ $# -ne 7 ]; then
    ...
    echo ""
    echo "     example: $0 ce07shsm D00004 44.63929 -124.30404 mfn/ctdbp 87 20161012.ctdbp2.json"
    exit 1
fi
PLATFORM=${1,,}
DEPLOY=${2^^}
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_ctdbp -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LON -dp $DEPTH -i $IN -o $OUT
fi
