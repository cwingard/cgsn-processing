#!/bin/bash
#
# Read the parsed DOSTA data files from the Endurance Surface Moorings and
...
if [ -e $IN ]; then
    ...
    $PYTHON -m cgsn_processing.process.proc_dosta -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LNG -dp $DEPTH -i $IN -o $OUT
fi
