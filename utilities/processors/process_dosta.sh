...
if [ $# -ne 8 ]; then
    ...
    echo ""
    echo "     example: $0 ce02shsm D00004 44.63929 -124.30404 nsif/dosta ctdbp 7 20161012.dosta.json"
    exit 1
fi
PLATFORM=${1,,}
DEPLOY=${2^^}
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_dosta -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LON -dp $DEPTH \
      -i $IN -o $OUT -df $CTD -ba
fi
