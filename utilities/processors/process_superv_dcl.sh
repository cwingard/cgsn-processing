...
if [ $# -ne 7 ]; then
    ...
    echo ""
    echo "     example: $0 ce02shsm D00004 44.63929 -124.30404 buoy/superv/dcl12 0 20161012.superv.json"
    exit 1
fi
PLATFORM=${1,,}
DEPLOY=${2^^}
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_superv_dcl -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LNG -dp $DEPTH -i $IN -o $OUT
fi
