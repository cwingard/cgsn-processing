#!/bin/bash
#
# Read the parsed HYDGN data files from the Endurance Surface Moorings and
...
if [ -e $IN ]; then
    ...
    $PYTHON -m cgsn_processing.process.proc_hydgn -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LNG -i $IN -o $OUT
fi
