...
IN="$DATA/parsed/$PLATFORM/$DEPLOY/$HYDGN/$FILE"
OUT="$DATA/processed/$PLATFORM/$DEPLOY/$HYDGN/${FILE%.json}.nc"
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_hydgn -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LON -dp 0 -i $IN -o $OUT
fi
