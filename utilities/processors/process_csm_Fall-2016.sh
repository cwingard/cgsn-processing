...
$PROCESS/process_phsen.sh $PLATFORM $DEPLOY $LAT $LNG "nsif/phsen" $FNAME.${PHSEN1[0]}.json
#--> SPKIR
...
if [ $MFN_FLAG == 1 ]; then
    ...
    $PROCESS/process_phsen.sh $PLATFORM $DEPLOY $LAT $LNG "mfn/phsen" $FNAME.${PHSEN2[0]}.json
    #--> PRESF
    #--> VEL3D
    #--> ZPLSC
fi
