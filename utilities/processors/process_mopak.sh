...
IN="$DATA/proc/$PLATFORM/$DEPLOY/$MOPAK/$FILE"
OUT="$DATA/erddap/$PLATFORM/$DEPLOY/$MOPAK/${FILE%.json}.nc"
...
if [ -e $IN ]; then
    ...
    $PYTHON -m cgsn_processing.process.proc_mopak -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LNG -i $IN -o $OUT
fi
