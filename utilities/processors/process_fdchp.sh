...
IN="$DATA/parsed/$PLATFORM/$DEPLOY/$FDCHP/$FILE"
OUT="$DATA/processed/$PLATFORM/$DEPLOY/$FDCHP/${FILE%.json}.nc"
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_fdchp -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LON -i $IN -o $OUT
fi
