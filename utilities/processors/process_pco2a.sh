#!/bin/bash
#
# Read the parsed PCO2A data files from the CGSN Coastal Surface Moorings and
...
IN="$DATA/parsed/$PLATFORM/$DEPLOY/$PCO2A/$FILE"
OUT="$DATA/processed/$PLATFORM/$DEPLOY/$PCO2A/${FILE%.json}.nc"
