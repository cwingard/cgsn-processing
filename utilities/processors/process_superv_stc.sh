...
if [ $# -ne 6 ]; then
    ...
    echo ""
    echo "     example: $0 ce09ospm D00006 46.85165 -124.98229 buoy/superv 20161012.syslog.json"
    exit 1
fi
PLATFORM=${1,,}
DEPLOY=${2^^}
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_superv_stc -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LON -dp 0 -i $IN -o $OUT
fi
