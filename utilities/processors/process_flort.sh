...
if [ $# -ne 8 ]; then
    ...
    echo ""
    echo "     example: $0 ce02shsm D00004 44.63929 -124.30404 nsif/flort 7 1153 20161012.flort.json"
    exit 1
fi
PLATFORM=${1,,}
DEPLOY=${2^^}
...
DEPTH=$6
SERIAL=${7^^}
CFILE="${PLATFORM^^}_$DEPLOY_FLORT.coeff"
FILE=`/bin/basename $8`
...
if [ -e $IN ] && [ ! -e $OUT ]; then
    ...
    $PYTHON -m cgsn_processing.process.proc_flort -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LNG -dp $DEPTH -i $IN -o $OUT -cf $COEFF -sn $SERIAL
fi
