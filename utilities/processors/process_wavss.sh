...
IN="$DATA/parsed/$PLATFORM/$DEPLOY/$WAVSS/$FILE"
OUT="$DATA/processed/$PLATFORM/$DEPLOY/$WAVSS/${FILE%.json}.nc"
...
if [ -e $IN ]; then
    ...
    python -m cgsn_processing.process.proc_wavss -p $PLATFORM -d $DEPLOY -lt $LAT -lg $LON -dp 0 -i $IN -o $OUT
fi
