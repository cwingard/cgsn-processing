...
$PROCESS/process_hydgn.sh $PLATFORM $DEPLOY $LAT $LON "buoy/hydgn1" $FNAME.hyd1.json
$PROCESS/process_hydgn.sh $PLATFORM $DEPLOY $LAT $LON "buoy/hydgn2" $FNAME.hyd2.json
...
$PROCESS/process_psc.sh $PLATFORM $DEPLOY $LAT $LON "buoy/psc" $FNAME.pwrsys.json
...
if [ $MFN_FLAG == 1 ]; then
    ...
    $PROCESS/process_mpea.sh $PLATFORM $DEPLOY $LAT $LON "mfn/mpea" $MFN_DEPTH $FNAME.pwrsys.json
