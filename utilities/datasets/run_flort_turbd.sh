#!/bin/bash -e
#
# Used to process flort data for turbidity calculations
# ppw 02082024
#
...
# Process the files "for turbidity"
echo "Processing $PLATFORM/$DEPLOY parsed flort data for turbidity"
for file in $PROC/$PLATFORM/$DEPLOY/$SUBASY/flort/*.json; do
    FNAME=`basename $file`
    echo "$FNAME"
    $PROCESS/process_flort_turbd.sh $PLATFORM $DEPLOY $LAT $LON "$SUBASY/flort" $CTD $DEPTH $SERIAL $FNAME
done
