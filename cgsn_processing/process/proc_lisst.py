...
def proc_lisst(infile, platform, deployment, lat, lon, depth):
    ...
    lisst = update_dataset(lisst, platform, deployment, lat, lon, [depth, depth_min, depth_max], LISST)
...
def main(argv=None):
    ...
    if lisst:
        lisst.to_netcdf(outfile, mode='w', format='NETCDF4', engine='netcdf4', encoding=ENCODING)
