...
def main():
    ...
    # calculate a depth record from the CTD pressure record
    df['depth'] = -1 * z_from_p(df['pressure'], lat)

    # re-calculate the practical salinity of the seawater from the pressure, temperature and conductivity measurements
    ...
    # setup some further parameters for use with the OMT class
    df['deploy_id'] = deployment
    profile_id = re.sub('\D+', '', fname)
    df['profile_id'] = "{}.{}".format(profile_id[1:4], profile_id[4:])
    df['x'] = lon
    df['y'] = lat
    df['z'] = depth             # uses the site depth
    df['t'] = df.pop('time')    # renames time to t, OMT class will convert back
    df['station'] = 0

    # make sure all ints are represented as int32 instead of int64
    df = reset_long(df)

    # Setup the attributes for the NetCDF file
    ctdpf_attr = CSPP
