...
# -*- coding: utf-8 -*-
"""
@package cgsn_processing.process.proc_superv_dcl
@file cgsn_processing/process/proc_superv_dcl.py
@author Christopher Wingard
@brief Creates a NetCDF dataset for the DCL Supervisor from JSON formatted source data
"""
import numpy as np
import os
import re
...
from cgsn_processing.process.error_flags import SupervErrorFlagDCL, derive_multi_flags
from cgsn_processing.process.configs.attr_superv_dcl import SUPERV
...
def main():
    ...
    df = derive_multi_flags(SupervErrorFlagDCL, 'error_flags', df)

    # Setup the global attributes for the NetCDF file and create the NetCDF timeseries object
    global_attributes = {
        'title': 'Mooring DCL Supervisor Data',
        'summary': (
            'Measures the status of the DCL, covering system voltages, current draws, leak detects and status of'
            'attached instrument ports.'
        ),
